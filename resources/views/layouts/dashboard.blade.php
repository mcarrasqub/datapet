        ...
        <div class="d-flex justify-content-between align-items-center px-4">
            ...
            @auth
                <form action="{{ route('logout') }}" method="POST" class="m-0">
                    @csrf
                    <button type="submit"
                        class="btn btn-outline-pet-green fw-medium rounded-pill d-flex align-items-center px-3 py-1">
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navLinks">
                <ul class="navbar-nav w-100 justify-content-around align-items-center">
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('dashboard.index') ? 'active-pill' : 'text-dark fw-medium' }}"
                            href="{{ route('dashboard.index') }}"><i class="bi bi-activity me-2"></i>Inicio</a>
                    </li>
                    ...
                    @if(auth()->check() && auth()->user()->role === 'admin')
                        <li class="nav-item">
                            <a class="nav-link {{ request()->routeIs('statistics.*') ? 'active-pill' : 'text-dark fw-medium' }}"
                                href="#"><i class="bi bi-graph-up me-2"></i>Estadísticas</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link {{ request()->routeIs('tasks.*') ? 'active-pill' : 'text-dark fw-medium' }}"
                                href="{{ route('tasks.index') }}"><i class="bi bi-check2-square me-2"></i>Tareas
                                Doctores</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link {{ request()->routeIs('users.*') ? 'active-pill' : 'text-dark fw-medium' }}"
                                href="{{ route('users.index') }}"><i class="bi bi-people me-2"></i>Usuarios</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link {{ request()->routeIs('register') ? 'active-pill' : 'text-dark fw-medium' }}"
                                href="{{ route('register') }}"><i class="bi bi-person-lines-fill me-2"></i>Registrar
                                Clientes</a>
                        </li>
                        <li class="nav-item">
